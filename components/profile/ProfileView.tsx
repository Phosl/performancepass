...
import { disciplines, frequencies, goals, levels, type AthleteProfile } from "@/lib/types";
import { membershipPlanById } from "@/lib/membership";
import { VideoCard } from "@/components/cards/VideoCard";
import styles from "./profile.module.scss";

type EditableProfile = Pick<AthleteProfile, "name" | "discipline" | "level" | "goal" | "frequency">;

export function ProfileView() {
  const { profile, hydrated, updateProfile, resetProfile } = useAthlete();
  const [form, setForm] = useState<EditableProfile>(() => ({ name: profile.name, discipline: profile.discipline, level: profile.level, goal: profile.goal, frequency: profile.frequency }));
  ...
  useEffect(() => {
    if (hydrated) setForm({ name: profile.name, discipline: profile.discipline, level: profile.level, goal: profile.goal, frequency: profile.frequency });
  }, [hydrated, profile.name, profile.discipline, profile.level, profile.goal, profile.frequency]);
  ...
  return (
    <div className={`shell ${styles.layout}`}>
      <aside className={styles.sidebar}>
        ...
        <h1>{profile.name}</h1>
        <p>{profile.discipline} · {profile.level}</p>
        ...
        <nav aria-label="Sezioni profilo">
          <a href="#dati" className={styles.active}><UserCircle size={18} /> Profilo atleta</a>
      ...
      <div className={styles.main}>
        <section id="dati" className={styles.panel}>
          <div className={styles.panelTitle}><div><p>LE TUE PREFERENZE</p><h2>Profilo atleta</h2></div><span>Usiamo questi dati per scegliere tecnica e percorsi.</span></div>
          <form onSubmit={save}>
            ...
            <label>Specialità principale<select value={form.discipline} onChange={(event) => field("discipline", event.target.value as AthleteProfile["discipline"])}>{disciplines.map((item) => <option key={item}>{item}</option>)}</select></label>
